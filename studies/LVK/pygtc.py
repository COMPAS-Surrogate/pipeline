...
def plotGTC(chains, **kwargs):
    ...
    # Set up some colors
    truthsDefaultColors = ['tab:orange', '#4d4d4d', '#4d4d4d']
    ...
    colorsDict = {
                ...
                'cyans': ('#17becf', '#17becf', '#17becf'),
              }
    defaultColorsOrder = ['blues', 'purples', 'cyans', 'oranges', 'greens', 'reds', 'purples',
                          'browns', 'pinks', 'grays', 'yellows']
    ...
    reference_label = kwargs.pop('reference_label', None)
    ...
    confLevels =  kwargs.pop('confLevels', (.3173, .0455, .0027))
    ...
    if (chainLabels is not None) or (truthLabels is not None):
        ...
        if reference_label is not None:
            ax.plot(0, 0, lw=4, color=REF_COLOR, label=reference_label)
            labelColors.append('darkgray')
...
def __plot1d(ax, nChains, chains1d, weights, nBins, smoothingKernel,
    ...
    bins = np.linspace(np.nanmin(chains1d), np.nanmax(chains1d), nBins)
    for k in reversed(range(nChains)):
        ...
        if not np.isnan(chains1d[k]).all():
            ...
            ax.hist(chains1d[k], weights=weights[k],
                    bins=bins, histtype='step', color=colors[k][1], density=True)
    ...
    if ref1d is not None:
        hist1d, edges = np.histogram(ref1d,
...
def __plot2d(ax, nChains, chains2d, weights, nBins, smoothingKernel,
    ...
    data = np.array(chains2d)
    bins = [
        np.linspace(np.min(data[:, 0,:]), np.max(data[:, 0,:]), nBins+1),
        np.linspace(np.min(data[:, 1,:]), np.max(data[:, 1,:]), nBins+1)
    ]
    ...
    for k in reversed(range(nChains)):
        ...
        if (np.isnan(chains2d[k][0]).all()) | (np.isnan(chains2d[k][1]).all()):
            ...
            hist2d, xedges, yedges = np.histogram2d(
                chains2d[k][0], chains2d[k][1], weights=weights[k], bins=bins)
            ...
            if filledPlots:
                ...
                ax.contourf(xbins, ybins, plotData[-1], levels=chainLevels[k],
                            colors=colors[k][:nContourLevels][::-1],alpha=0.5)

...
def update_parameter_table_with_acquisition_function(train_data, outdir):
    ...
    new_asf = sample_next_hyperparameter(expected_improvement, model, train_data.lnl.values, greater_is_better=True, bounds=np.array([[0, 0.2]]), n_restarts=25)
...
def sample_next_hyperparameter(acquisition_func, model, evaluated_loss, greater_is_better=True,
                               bounds=[0, 0.2], n_restarts=25):
